...
namespace GnossBase
{
	...
	public class GnossOCBase : IGnossOCBase
	{
		...
		public string GNOSSID
		{
			...
			{
				this.mGNOSSID = value;
				int ultimaBarra = this.mGNOSSID.LastIndexOf('/');
				int barraBaja = this.mGNOSSID.LastIndexOf('_');
				int longitudGuid = barraBaja - ultimaBarra;
				Guid nuevoResource = Guid.Parse(this.mGNOSSID.Substring(ultimaBarra + 1, longitudGuid));
				Guid nuevoArticle = Guid.Parse(this.mGNOSSID.Substring(barraBaja + 1));
